    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < codedText.Length; i++)
            {
                ...
                {
                    if (char.IsDigit(codedText[i+1]))
                    {
                        repeatance = int.Parse(codedText[i].ToString()) * 10 + int.Parse(codedText[i].ToString());
                        string replacement = new string(codedText[i + 2], repeatance - 1);
                        codedText.Remove(i, 2);
                        codedText.Insert(i, replacement);
