...
function updateEntities(dt) {
    ...
    for (var i = 0; i < bullets.length; i++) {
        ...
        if (bullet.pos[1] < 0 || bullet.pos[1] > canvas.height || bullet.pos[0] > canvaas.width) {
            bullets.splice(i, 1);
            i--;
        }
    }
    ...
    for (var i = 0; i < explosions.length; i++) {
        explossions[i].sprite.update(dt);
...
function boxCollides(pos, size, pos2, size2) {
    return collodes(pos[0], pos[1], pos[0] + size[0], pos[1] + size[1],
