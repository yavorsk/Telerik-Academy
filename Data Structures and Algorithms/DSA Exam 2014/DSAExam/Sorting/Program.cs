    ...
    class Program
    {
        ...
        static void Main()
        {
            ...
            while (queue.Count > 0)
            {
                ...
                for (int i = 0; i <= N - K; i++)
                {
                    HandleArr2(currentSeq.Arr, i, K);
                    if (!used.Contains(string.Join("", currentSeq.Arr)))
                    {
                        var nextSeq = new Sequence(currentSeq.Arr);
                        nextSeq.QueueLevel = currentSeq.QueueLevel + 1;
                        queue.Enqueue(nextSeq);
                        used.Add(string.Join("", currentSeq.Arr));
                    }
                    HandleArr2(currentSeq.Arr, i, K);
                }
            }
